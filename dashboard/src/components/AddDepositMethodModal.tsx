...
interface AddDepositMethodModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: () => void;
}

function AddDepositMethodModal({
  isOpen,
  onClose,
  onSuccess,
}: AddDepositMethodModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(
        "http://localhost:8000/api/deposit-methods",
        {
          method: "POST",
          headers: {
      ...
      console.log("Deposit method created successfully");
      resetForm();
      onSuccess();
      onClose();
    } catch (err) {
      const errorMessage =
        err instanceof Error ? err.message : "Failed to create deposit method";
    ...
    <div className="modal-overlay" onClick={handleClose}>
      ...
      >
        <div className="modal-header">
          <h2 className="modal-title">Add Deposit Method</h2>
          <button
...
export default AddDepositMethodModal;
