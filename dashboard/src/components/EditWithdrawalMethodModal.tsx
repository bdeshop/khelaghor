...
interface EditWithdrawalMethodModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: () => void;
...
function EditWithdrawalMethodModal({
  isOpen,
  onClose,
  onSuccess,
  withdrawalMethod,
}: EditWithdrawalMethodModalProps) {
  ...
  useEffect(() => {
    if (withdrawalMethod) {
      setMethodNameEn(withdrawalMethod.methodNameEn || "");
      setMethodNameBn(withdrawalMethod.methodNameBn || "");
      setMinimumWithdrawal(
        withdrawalMethod.minimumWithdrawal?.toString() || ""
      );
      setMaximumWithdrawal(
        withdrawalMethod.maximumWithdrawal?.toString() || ""
      );
      setProcessingTime(withdrawalMethod.processingTime || "");
      setStatus(withdrawalMethod.status || "Active");
      setWithdrawalFee(withdrawalMethod.withdrawalFee?.toString() || "");
      setFeeType(withdrawalMethod.feeType || "Fixed");
      setTextColor(withdrawalMethod.colors?.textColor || "#000000");
      setBackgroundColor(withdrawalMethod.colors?.backgroundColor || "#ffffff");
      setButtonColor(withdrawalMethod.colors?.buttonColor || "#007bff");
      setInstructionEn(withdrawalMethod.instructionEn || "");
      setInstructionBn(withdrawalMethod.instructionBn || "");
      setUserInputFields(withdrawalMethod.userInputFields || []);
      setMethodImagePreview(
        withdrawalMethod.methodImage
          ? `http://localhost:8000${withdrawalMethod.methodImage}`
          : ""
      );
      setWithdrawPageImagePreview(
        withdrawalMethod.withdrawPageImage
          ? `http://localhost:8000${withdrawalMethod.withdrawPageImage}`
          : ""
      );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(
        `http://localhost:8000/api/withdraw-methods/${withdrawalMethod.id}`,
        {
          method: "PUT",
          headers: {
    ...
    <div className="modal-overlay" onClick={handleClose}>
      ...
      >
        <div className="modal-header">
          <h2 className="modal-title">Edit Withdrawal Method</h2>
          <button
        ...
        <form onSubmit={handleSubmit} className="modal-form">
          ...
          <div className="form-row">
            ...
            <div className="form-group">
              ...
              {!withdrawPageImageFile && withdrawPageImagePreview && (
                <span className="form-hint">
                  Keep current image or upload a new one
                </span>
              )}
...
export default EditWithdrawalMethodModal;
